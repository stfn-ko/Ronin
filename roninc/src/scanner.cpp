...
std::unordered_map<std::string, token_t> keyword_map = {
    {"/r", KW_PERM_R},      {"/rx", KW_PERM_RX},
    {"/rs", KW_PERM_RS},    {"/rw", KW_PERM_RW},
    {"/rwx", KW_PERM_RWX},  {"/rws", KW_PERM_RWS},

    {"bool", KW_TYPE_BOOL}, {"isize", KW_TYPE_ISIZE},
    {"i32", KW_TYPE_I32},   {"usize", KW_TYPE_USIZE},
    {"u32", KW_TYPE_U32},   {"f32", KW_TYPE_F32},
    {"str", KW_TYPE_STR},
    ...
    {"::", COMB_ACCESS},    {">=", COMB_GTOE},
    {"<=", COMB_LTOE},      {"==", COMB_EQ},
    {"!=", COMB_NEQ},       {"->", COMB_RETURN},
    {"+=", COMB_MINUS_EQ},  {"-=", COMB_PLUS_EQ},
    {"++", COMB_ADD},       {"--", COMB_SUB},
    {"&&", COMB_AND},       {"||", COMB_OR},

    {"@", PUNCT_AT},        {"/", PUNCT_FW_SLASH},
    {"\\", PUNCT_BC_SLASH}, {"!", PUNCT_EXCLM},
    {"#", PUNCT_HTAG},      {"%", PUNCT_PRCNT},
    {"&", PUNCT_AMPRSND},   {"*", PUNCT_ASTER},
    {"(", PUNCT_L_PAREN},   {")", PUNCT_R_PAREN},
    {"{", PUNCT_L_SQUIG},   {"}", PUNCT_R_SQUIG},
    {"[", PUNCT_L_SQRBR},   {"]", PUNCT_R_SQRBR},
    {"'", PUNCT_SQUOTE},    {"\"", PUNCT_DQUOTE},
    {";", PUNCT_SEMI},      {":", PUNCT_COL},
    {">", PUNCT_GT},        {"<", PUNCT_LT},
    {"=", PUNCT_EQ},        {"-", PUNCT_MINUS},
    {"+", PUNCT_PLUS},      {".", PUNCT_POINT},
    {",", PUNCT_COMMA},
};
...
void add_token(std::vector<token> &tokens, const std::string &str,
               position &pos, token_t type = token_t::UNDEFINED) {
  tokens.emplace_back(
      token{str, type != token_t::UNDEFINED ? type : get_type(str), pos});

  pos.col += str.size();
}

void get_id(file_reader &fr, std::vector<token> &tokens, std::string &str,
            position &pos) {
...
void get_number_lit(file_reader &fr, std::vector<token> &tokens,
                    position &pos) {
...
void get_string_lit(file_reader &fr, std::vector<token> &tokens,
...
void get_combo_token(file_reader &fr, std::vector<token> &tokens,
                     position &pos) {
...
void get_permission_token(file_reader &fr, std::vector<token> &tokens,
                          std::string &str, position &pos) {
